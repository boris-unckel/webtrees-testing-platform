...
| S45 | Report-Primitive HTML | — | `ReportHtmlObjectsIntegrationTest` [Spec-C] ✅ *(spezifikationsbasiert+strukturbasiert, 13 Tests: fill/border/newline Assertions TextBox+Cell; auf neue Report-API portiert — Enums/Style/ReportConfig)* + `RomanNumeralsIntegrationTest` [CRAP] ✅ *(RomanNumeralsService numeric/roman Konversion)*. **PDF-Primitive entfernt:** `ReportPdfObjectsIntegrationTest` (10 Tests) durch Upstream PR #5389 obsolet — Renderer-Interna (tcpdf/lastCellHeight) jetzt `private`, isoliertes Element-Rendering nicht mehr öffentlich testbar | — | OK | — |
