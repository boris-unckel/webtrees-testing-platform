...
| Teststufe 2 — Komponentenintegrationstest (Dateisystem) | G01–G04, G07–G10, G12–G16, G24 (14) | S01–S03, S05–S08, S10–S12, S19, S21, S22, S53 (14) | P01–P24, P27–P29, P30, P37, P38, P42 (31) | SEC-H01–H02, SEC-D01–D02, SEC-C01–C03, SEC-PUB01, SEC-WZ03 (9) | E01–E08 (8), A01–A08, A15, A16 (10), K01, K02 (2) | **88** |
| Teststufe 3 — Systemtest (HTTP/Playwright) | G20, G21 (2) | S05–S10, S13–S18, S20, S23–S24, S26–S41, S46, S47, S50, S53 (35) | P01–P03, P14–P19, P22, P24–P30, P37, P38, P40, P41 (21) | SEC-H03–H06, SEC-M01–M03, SEC-PUB02–PUB04, SEC-W01, SEC-WZ01–WZ04, SEC-HDR01–HDR04 (18) | E01–E06, E08 (7), A01, A04, A05, A07, A08 (5), K01, K02 (2), M16 (1) | **91** |
| **Nur Teststufe 2** | — | — | P04–P13, P20–P21, P23, P42 (14) | SEC-H01–H02, SEC-D01–D02, SEC-C01–C02, SEC-PUB01 (7) | A15, A16 (2) |
| **Nur Teststufe 3** | — | — | P25, P26 (2) | SEC-H03–H06, SEC-M01–M03, SEC-PUB02–PUB04, SEC-W01, SEC-WZ01–WZ02, SEC-WZ04, SEC-HDR01–HDR04 (17) | — |
| **Beide Teststufen** | — | — | 14 Features (P01–P03, P14–P19, P22, P24, P27–P29) | SEC-C03, SEC-WZ03 (2) | K01, K02 (2) |
| **Summe** | **24** | **40** | **30** | **26** | **121** |
...
| **Anwendungsfall-Test** | G20, G21, S05–S10, S13–S18, S23–S24, S26–S41, S46, S47, S50, S53, E01–E06, E08, K01, K02, M16 | Systemtest-Szenarien mit Nutzerinteraktion: Import-Export-Roundtrip, Chart-Rendering, Seitennavigation, Record-Seiten, Auth-Formulare, Kalender, Suche (Felder/Datum/Phonetisch/Paginierung), Statistik, Homepage-Blöcke, Interaktiver Stammbaum, Hilfetexte, Datenerfassung (Personen/Fakten/Raw-GEDCOM/Nebenrecords/Medien/Reorder/TomSelect), Kontaktformular, Nachrichten, Legacy-URL-Weiterleitungen, Error-Page-Rendering |
...
| **Anwendungsfall-Test** | P25–P30, P37, P38, P40, P41, A01, A04, A05, A07, A08 | End-to-End-Szenarien: Seitenaufruf → Sichtbarkeitsprüfung → Edit → Pending Change → DB-Persistenz; Merge-Workflow, Benutzer-Admin, Account-Selbstverwaltung, Pending-Changes-Workflow, Stammbaum-Management, Präferenzen, Modul-Konfiguration, Benutzerverwaltung, Medienverwaltung |
...
| **Äquivalenzklassenbildung** | A12 | SiteOffline CLI-Command via Symfony `CommandTester`: EP1 ohne Message → SUCCESS + `isOffline()`, EP2 Custom-Message → `message()` korrekt, EP3 Spezialzeichen (HTML/Quotes) → kein Escaping-Verlust, EP5 existierende Datei → Überschreiben (MaintenanceModeService als Postcondition-Orakel) |
| **Äquivalenzklassenbildung** | A13 | SiteOnline CLI-Command via Symfony `CommandTester`: EP1 Offline → Online + Datei gelöscht, EP2 bereits online → idempotent SUCCESS + "already online", EP4 Sequenz offline→online → korrekte Zustandsübergänge |
| **Äquivalenzklassenbildung** | A14 | ConfigIni CLI-Command via Symfony `CommandTester`: EP1/EP4 gültige DB-Credentials + base-url → SUCCESS + Config-Datei + DB-Verbindungstest; EP3 leere base-url → WARNING + SUCCESS; EP7 Trailing-Slashes → rtrim; EP8 dbverify=true → INI-Wert '1'; EP5 ungültige Credentials → FAILURE. Config-Backup/Restore im setUp/tearDown, DB-Reconnect im tearDown |
| **Äquivalenzklassenbildung** | G31 | TreeImport CLI-Command via Symfony `CommandTester`: EP1 gültiger Baum + Datei → SUCCESS + DB-Count > 0 (Individuen importiert); EP3 Baum-not-found → FAILURE + "not found"; EP4 Datei-not-found → FAILURE + "does not exist"; EP2 keep-media-Option → SUCCESS |
| **Äquivalenzklassenbildung** | M11 | Router-Middleware mit echtem Aura `RouterContainer`: B1/EP1 rewrite_urls + route-Query → 308-Redirect mit Link-Header; B5/EP5 POST an GET-Route → 405 + Allow-Header; B6/EP6 Accept-Mismatch → 406; B7/EP7 keine Route → Fallback-Handler; B4/EP4 Route-Match → Handler via Dispatcher-Pipeline (DummyRouterHandler); B8/B9/EP8 Tree-Parameter → DB-Auflösung + Container-Registrierung; B3/EP3 rewrite_urls=false → URI-Update aus Query |
| **Äquivalenzklassenbildung** | M03 | ClientIp-Middleware Proxy-Trust-Konfiguration: EP1 beide Attribute null → Durchlauf ohne Fehler; EP2 gültige CSV-Strings (trusted_headers, trusted_proxies) → Proxy-Trust konfiguriert; EP3 leere Strings → wie null; BVA CSV mit genau einem Element → korrekt verarbeitet; Handler-Response unverändert |
| **Äquivalenzklassenbildung** | M09 | BaseUrl-Middleware URL-Ermittlung: EP1/B1 base_url leer → Auto-Detection aus Request-URI (Scheme+Host); EP3/B4 base_url gesetzt → Scheme+Host aus konfigurierter URL übernommen; EP5/B4 expliziter Port → Port korrekt; EP4/B5 ohne Port → Default-Port (null in PSR-7) |
| **Äquivalenzklassenbildung** | M13 | UseLanguage-Middleware Sprachauswahl-Priorität: EP1/B1 Session-Language + Modul aktiv → Sprache aus Session; EP4/B4 kein Session-Wert → Fallback en-US; EP2/B2 ungültiger Language-Code → Fallback en-US; Handler-Response unverändert |
| **Äquivalenzklassenbildung** | M15 | ErrorHandler-Middleware PHP-Error-Konvertierung: EP1/B1 Error mit aktivem error_reporting → ErrorException (Severity + Message geprüft); EP2/B2 @-unterdrückter Error → ignoriert, Response normal; EP3/B3 kein Fehler → normaler Durchlauf; Error-Handler-Restaurierung nach normalem Request |
| **Äquivalenzklassenbildung** | M25 | RegisterGedcomTags-Middleware: EP1 registerTags() wird mit ElementFactory aufgerufen + Handler ausgeführt; Aufruf-Reihenfolge: registerTags() VOR Handler (Tracker-Assertion) |
| **Äquivalenzklassenbildung** | M26 | BootModules-Middleware: EP1 bootModules() wird mit Theme aufgerufen + Handler ausgeführt; Aufruf-Reihenfolge: bootModules() VOR Handler (Tracker-Assertion) |
| **Äquivalenzklassenbildung (Pragmatisch C)** | P42 | UserList CLI-Command via Symfony `CommandTester`: Benutzer-Listing auf STDOUT mit Spalten user_id, user_name, real_name, email, Admin/Verified/Approved-Status; Ausgabe-Format- und Inhalts-Validierung; 6 Tests, 34 Assertions |
| **Äquivalenzklassenbildung (Pragmatisch C)** | A15 | CompilePoFiles CLI-Command via Symfony `CommandTester`: `.po`-Dateien aus `resources/lang/` in PHP-Arrays kompilieren; Erfolgs- und Fehlerbehandlung; 2 Tests, 6 Assertions |
| **Äquivalenzklassenbildung (Pragmatisch C)** | A16 | TreeList CLI-Command via Symfony `CommandTester`: Stammbaum-Listing auf STDOUT mit Spalten gedcom_id, tree_name, tree_title, imported; Ausgabe-Format- und Inhalts-Validierung; 6 Tests, 45 Assertions |
| **Äquivalenzklassenbildung** | K01 | ContactForm HTTP-Handler: ContactPage GET→200 (Formular-Rendering); ContactAction POST mit CSRF/Captcha-Validierung, Empfänger-Auflösung, E-Mail-Versand (kein SMTP im Test-Stack: Response-Status und Redirect-Verhalten prüfen); Fehler-EPs: ungültiger Empfänger, leere Pflichtfelder, CSRF-Mismatch; 11 Tests, 34 Assertions |
| **Äquivalenzklassenbildung** | K02 | UserMessage HTTP-Handler: MessagePage GET→200 (Formular); MessageAction POST Nachricht an Nutzer senden→Redirect; MessageSelect Empfänger-Auswahl→200; Fehler-EPs: ungültiger Empfänger, nicht-authentifiziert, Nachricht an sich selbst; 9 Tests, 31 Assertions |
| **Äquivalenzklassenbildung** | M14 | UseTheme-Middleware Theme-Auswahl-Priorität: Theme aus Session/Siteprefs priorisieren → Container-Binding `ModuleThemeInterface`; EP-Partitionen für Theme-Quellen und Fallback-Verhalten; 4 Tests, 17 Assertions |
| **Smoke-Test** | M10 | LoadRoutes-Middleware Routen-Laden: Core-Routing-Tabellen (`ApiRoutes`, `WebRoutes`) laden und Router im DI-Container hinterlegen; Smoke-Prüfung der Route-Registrierung und Handler-Auflösung; 3 Tests, 10 Assertions |
| **Äquivalenzklassenbildung** | M17 | DebugLogger-Middleware Debug-Logging: SQL-Query-Zählung, Response-Time, Memory-Peak → Debug-Response-Header (nur wenn Debug-Flag aktiv); EP-Partitionen für Debug-Flag aktiv/inaktiv und Header-Prüfung; 4 Tests, 19 Assertions |
| **Äquivalenzklassenbildung (Pragmatisch C)** | M18 | DoHousekeeping-Middleware Housekeeping-Trigger: zufällig 1/1000 Requests → `HousekeepingService` aufrufen; Spec-C-Prüfung des Trigger-Mechanismus und der Service-Delegation; 3 Tests, 11 Assertions |
| **Smoke-Test** | M23 | CheckForNewVersion-Middleware Update-Prüfung: asynchron verfügbare webtrees-Versionen prüfen (nur bei GET-Requests) → Versions-Info im Container; Smoke-Prüfung des Durchlauf- und Response-Verhaltens; 4 Tests, 15 Assertions |
| **Spezifikationsbasiertes Testen (Spec-C)** | M16 | HandleExceptions-Middleware Exception-Handling: HttpException→passender HTTP-Status-Code, AJAX GET→200, FilesystemException→500, allgemeiner Throwable→500, Fehlermeldungen im Response-Body; 7 Tests, 22 Assertions |
| **Äquivalenzklassenbildung** | M19 | CompressResponse-Middleware Response-Kompression: gzip/deflate-Encoding-EP-Matrix, zlib-Extension-Prüfung, MIME-Type-Filterung (nur text/*-Typen komprimierbar), bereits-codierte Response→Skip, Accept-Encoding-Header-Varianten; 9 Tests, 34 Assertions |
| **Spezifikationsbasiertes Testen (Spec-C)** | M28 | EmitResponse-Middleware Response-Emittierung: Body-Emission an Client, Cache-Control-Header hinzufügen/beibehalten, FastCGI-Erkennung, leerer Body→kein Output; 5 Tests, 22 Assertions |
| **Äquivalenzklassenbildung** | A08 | AdminMediaManagement HTTP-Handler: ManageMediaPage render→200, ungültiger Pfad→Fehlerbehandlung, Path-Traversal-Sicherheitsprüfung, nicht-existierende Records→Fehlerbehandlung, FixLevel0MediaPage render→200; 5 Handler (AdminMediaFileDownload, FixLevel0MediaPage, FixLevel0MediaAction, ManageMediaPage, ManageMediaAction); 5 Tests, 17 Assertions |
| **Batch-Smoke + Äquivalenzklassenbildung** | S53 | LegacyUrlRedirect ~27 Redirect*Php-Handler: Individual/Family/Source/Note/Repository/GedRecord→301-Redirect, ungültiger Baum→410, ungültiger Record→410, Calendar/ReportEngine-Redirect, DEFAULT_GEDCOM-Fallback, Pedigree-Style-Mapping; EP-Partitionen für Record-Typen, Fehler-Klassen und Spezial-Handler; 13 Tests, 49 Assertions |
