...
| S44 | Report-Parser Erweitert *(spezifikationsbasiert, Pragmatisch C)* | ParserGenerate: Vorfahren-Bericht (EP1 addAncestors→non-empty HTML), Nachkommen-Bericht (EP3 addDescendancy→non-empty HTML), Individual-Bericht mit Fakten+Bild (EP7 factsStartHandler+imageStartHandler→non-empty HTML) | 3 | Mittel |
| S45 | Report-Primitive HTML *(spezifikationsbasiert+strukturbasiert)* | Html*: fill/border/newline Assertion-Tests (TextBox: bgcolor, border:solid, X-Pos; Cell: border='1'/'T'/''/ptp/bgcolor) — auf neue Report-API portiert (Enums/Style/ReportConfig, Upstream PR #5389). **PDF-Primitive entfernt:** PR #5389 kapselte tcpdf/lastCellHeight/largestFontHeight als `private` → isoliertes Element-Rendering nicht mehr öffentlich testbar; Abdeckung via Report-Modul-/Snapshot-Tests | 13 | Mittel |
